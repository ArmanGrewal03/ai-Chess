...
class Board:
    ...
    def move(self,piece,move,testing=False):
        ...
        ep_empty=self.squares[final.row][final.col].is_empty()
    ...
    def calc_moves(self,piece,row,col,bool=True):
        ...
        def pawn_moves():
            ...
            for possbile_move_col in possible_move_cols:
                if Square.in_range(possible_move_row,possbile_move_col):
                    if self.squares[possible_move_row][possbile_move_col].has_rival_piece(piece.color):
                        initial=Square (row,col)
                        final_piece=self.squares[possible_move_row][possbile_move_col].piece
                        final=Square(possible_move_row,possbile_move_col,final_piece)
                        move=Move(initial,final)
                        if bool:
                            if not self.in_check(piece,move):
                                piece.add_move(move)
    ...
    def _add_pieces(self,color):
        ...
        for cols in range(COL):
            self.squares[row_pawn][cols]=Square(row_pawn,cols,Pawn(color))
    ...
    def true_ep(self,piece):
        if not isinstance(piece, Pawn):
            return
        for row in range(ROWS):
            for col in range(COL):
                if isinstance(self.squares[row][col].piece,Pawn):
                    self.squares[row][col].piece.ep=False
